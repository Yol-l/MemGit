...
write_rules: |
  ...
  不记录代码细节（代码在仓库里）
  里程碑完成时更新 status
  进度条目超 6 月未更新移入「进度·归档」
  重新谈到已归档事件时从归档区取出放回活跃区
  项目结束时移入 archive/
---
...
- (07-14) 系统控制规则闭环：硬边界 + 膨胀阈值 + 审计摘要
- (07-14) 项目事件机制：进度归档 → 重新提及则复活
- (07-14) 策略从 skill 迁移到记忆仓库，机制可共享
- (07-13) 记忆合并规则（5 种）、渐进式加载、写入约束补全
- (07-13) SKILL.md 瘦身、自成长规则外置、动态 INDEX.md
- (07-13) 方案确定：Git 驱动 + Markdown + fork 分发

## 进度·归档

> 以下条目超 6 个月未活跃，已归档。重新谈到时移回「当前进度」。
