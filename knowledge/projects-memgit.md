...
### 2026-07-14: 约束规则补全（5 个缺口修复）
- **Context**: 检查约束规则发现 5 个缺口：新建文件无门槛、superseded 条目留在活跃区、条目无时间标记、删除规则太绝对、写入通知太吵
- **Decision**:
  1. 新建文件需被提及 2 次以上 + 无法归入已有文件
  2. superseded 条目立即移入存档区，活跃区只保留当前值
  3. 每条新条目首次写入时标注日期
  4. 用户明确要求时可以删除，但必须告知
  5. 多次写入在会话结束时汇总通知
- **Rationale**: 约束的精度决定记忆的质量。没有门槛的创建和没有规范的更新，会让记忆库变成垃圾场
