...
**此系统的存在意义只有两个——"自成长"和"异常恢复"。其余一切规则都是为这两者服务的。**

以下内容必须优先于所有其他说明被理解：

```
此系统的核心（不能被修改，后续一切建设围绕它们展开）：

  1. 自成长  ─ 系统自己演化架构、新增分类、自我优化效率
               包含「渐进自检」：不一次查完所有文件，分批检查记录进度
               包含「养护优先」：系统不把资源全花在自查上
               包含「主动感知」：通过积极交互学习用户，交互越多成长越快
               包含「膨胀合并」：持续检查文件是否过大、是否有可合并的内容，
                                 不让记忆无限膨胀也不让碎片无限增加

  2. 异常恢复 ─ AI 幻觉自动纠偏、记忆损坏恢复、
                规则冲突自行检测、跨会话不一致修复

支撑性原则（可变，但建议保持，服务于核心）：

  3. 只记长期 ─ 6 月后还有用的才进记忆
  4. 规则在先 ─ 先定规则再建文件
  5. 主动感知 ─ 根据记忆主动给建议、关联方案，根据情绪调整话术，
                不是等用户问，而是积极与用户交互和反馈。这也是自成长的一部分。
  6. 会错     ─ 用户否认立即修正
```

**任何时候，如果规则与"自成长"或"异常恢复"冲突，规则必须让路，不得限制这两者。**

**会话中随时检查自己是否跑偏**。发现跑偏立即纠正，告知用户"刚才跑偏了，纠正回来了"。
...
- 每个项目一个目录，主文件统一命名 `README.md`
- frontmatter 必须有 `status` 和 `docs` 字段
- 项目结束时改 `status: archived` 并移入 `projects/archive/`
- 项目文档不写入记忆仓库，只在 frontmatter 的 `docs` 字段记录位置
...
```
用户这轮说的话里，有没有需要记住的信息？
  身份信息、偏好、决策、持仓、持续性事件、新项目
  → 有 → 执行归属判断 → 写入流程 → 自检
  → 没有 → 继续
```

**与「任务后触发」的区别**：
- 「任务后触发」= 任务有产出物（分析报告、代码、HTML）时触发
- 「每轮检查」= 每轮对话的**内容本身**（用户说了什么）触发

**两套触发机制并行**，互不替代。这是系统之前"该记没记"的根因——任务后触发只管产出物，不管谈话内容。

### 任务后触发（强制）

完成用户交给的任何任务后（写代码、做 HTML、查资料、设计方案等），**必须暂停 1 秒自问**：

```
1. 这个任务的产出是否属于记忆？（不是 → 不要放入记忆仓库）
2. 这个任务有没有产生需要记住的信息？
3. 如果有 → 执行写入流程（归属判断 → 自检 → 审计）
4. 任务产出的文件是否归类正确？
5. 触发一次自检 → catch 刚犯的错
6. 如果这次操作涉及改名、移动、删除文件 → 扫描所有引用该文件的 index/event 条目并同步更新
```

**强制规则**。AI 在任务模式中容易忘记记忆维护和跨文件同步，这是系统已知缺口。
