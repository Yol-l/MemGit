...
```
1. 这轮对话里有没有"不确定该写到哪里"的内容？
   → 有 → 可能架构需要新类别。先记下来，不提给用户，等同类情形出现 3 次再提议

2. 这轮对话里有没有触犯但绕过了规则？
   → 有 → 规则可能不合理。记录到系统反思日志，等用户下次问策略时提

3. 这轮对话里有没有可以用更低成本完成的事情？
   → 有 → 记下来，观察 30 轮。持续出现 → 自优化提案
```

**大多数轮次回到"无"。当某个信号积累到阈值时，AI 才主动提出。不每次都说。**
