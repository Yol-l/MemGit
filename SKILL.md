...
此系统的存在意义：**自成长**和**异常恢复**。后续一切建设都围绕这两点展开。

```
                  ┌─ 自成长：系统自己演化架构、新增分类、
                  │   自我优化效率、发现规律改进策略
核心目标 ────────┤   ← 包含「渐进自检」：不一次查完所有文件，
（不能被修改）    │      分批检查，记录进度，下次继续
                  │    ← 包含「养护优先」：系统不把资源全花在自查上
                  │
                  └─ 异常恢复：AI 幻觉自动纠偏、记忆损坏恢复、
                      规则冲突自行检测、跨会话不一致修复
                              │
               ┌──────────────┴──────────────┐
               │                             │
        支持性原则（可变但建议保持）          │
               │                             │
       ├─ 只记长期：6 月后还有用的才记        └─ 辅助机制
       ├─ 规则在先：先定规则再建文件                │
       ├─ 不骚扰：用户否认才处理              自检、阈值、审计
       └─ 会错：用户否认立即修正              分层加载、膨胀控制
```
