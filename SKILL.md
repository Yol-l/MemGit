...
每条条目首次写入标注日期和信心等级：`确认的`（你亲口说）、`推断的`（AI 猜的）、`存疑的`（你说"可能"）、`过期的`（1 年没确认了）。引用"推断的"时语气轻一些。

**explicit vs inferred 的判定**：用户直接陈述偏好（"我喜欢X""我决定用X"）= explicit。用户使用某技术但不表态（"帮我写X代码"）= inferred，需确认 2 次。

### 记忆自愈（核心防幻觉机制）

记忆不是"事实"，是"待验证的断言"。AI 通过两种方式自然修正：

**你否认时**：
```
你否认 → 立即修正该条 → 同一次会话中写的其他条目一并进行降级为「存疑的」
→ 不再引用降级的条目，直到你重新确认
```

**推断的记忆自然而然检验**：
```
用户聊到相关话题时，AI 自然地试探："你是喜欢 X 对吧？"
你确认 → 升级为「确认的」
你否认 → 修正
你只说"嗯" → 不处理
```

**不做定期审问**。不在用户没提相关话题时突然问"你以前是不是喜欢跑步？"——那是骚扰。
