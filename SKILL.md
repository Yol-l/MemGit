...
**源文保留**：写入 explicit 条目时，保留用户原始话语作为 `[source: "..."]`。未来 AI 可以对比源文和记录是否一致。

### 记忆读时校验（核心防幻觉机制）

记忆不是"事实"，是"待验证的断言"。每轮对话中，AI 使用记忆时同步校验：

```
1. 引用记忆时观察用户反应
   → 用户没否认 → 记忆可能正确，confidence +1（内部计数，不写入）
   → 用户否认/纠正 → 记忆错误，立即修正 + 标记 source 核查

2. 会话启动时交叉检查
   → 同一文件内是否有互相矛盾的条目？
   → explicit 条目的 [source] 原文与记录的释义是否一致？
   → inferred 条目是否已被用户行为否定？

3. 发现可疑记忆时的处理
   → 标记 flag: needs-review
   → 在对话中自然地问用户："我记着你之前说过 X，还是这样吗？"
   → 用户确认 → 移除 flag
   → 用户否认 → 纠正合并（self-growth.md 第 3 种）
   → 用户不置可否 → 降级为 uncertain
```

**这个机制是重中之重**：AI 有幻觉，写入时可能出错。读时校验是唯一能在后续会话中发现并修复幻觉的手段。每一轮对话都是一次校验机会。
